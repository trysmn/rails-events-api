...
time_now = Time.new(2018, 9, 17, 9, 0, 0, "+01:00")

# we then create 10 events with the same start and end times,but each event is on the next day after the last one:
10.times do
  Event.create(start_time: time_now.iso8601(0), end_time: (time_now + (60 * 60 * 7)).iso8601(0), label: Faker::Ancient.god, category: Faker::Color.color_name )
  time_now += 60 * 60 * 24
end
